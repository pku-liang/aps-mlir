 ...
 */
static inline uint32_t pack_dma_rs2(uint32_t spm_tile_idx, uint32_t dram_row_stride) {
    // SPM address = tile_idx * tile_bytes, then >> 2 for 4-byte units
    uint32_t spm_addr = spm_tile_idx * TILE_SIZE * TILE_SIZE;
    uint32_t spm_field = spm_addr << 16;  // bits [31:16]

    return spm_field | dram_row_stride;
}
...
void load_matrix_a(uint32_t dram_base) {
    ...
    for (int i_tile = 0; i_tile < M_TILES; i_tile++) {
        for (int k_tile = 0; k_tile < K_TILES; k_tile += TILES_PER_DMA) {
            ...
            rs1 = dram_addr;
            rs2 = pack_dma_rs2(spm_tile_idx, A_DRAM_STRIDE);  // 64 bytes per i8 tile
...
void load_matrix_b(uint32_t dram_base) {
    ...
    for (int k_tile = 0; k_tile < K_TILES; k_tile++) {
        for (int j_tile = 0; j_tile < N_TILES; j_tile += TILES_PER_DMA) {
            ...
            rs1 = dram_addr;
            rs2 = pack_dma_rs2(spm_tile_idx, B_DRAM_STRIDE);
...
void store_matrix_c(uint32_t dram_base) {
    ...
    for (int i_tile = 0; i_tile < M_TILES; i_tile++) {
        for (int j_tile = 0; j_tile < N_TILES; j_tile += TILES_PER_DMA) {
            ...
            rs1 = dram_addr;
            rs2 = pack_dma_rs2(spm_tile_idx, C_DRAM_STRIDE);  // 128 bytes per i16 tile
...
int8_t A[M * K] __attribute__((aligned(128)));

// Matrix B: 128x128 i8 = 16KB
int8_t B[K * N] __attribute__((aligned(128)));

// Matrix D (output): 128x128 i16 = 32KB
int16_t D[M * N] __attribute__((aligned(128)));
...
void gemm_128x128_full(void) {
    ...
    // {
    //     // For i16 elements: col offset = tile_col * TILE_SIZE * 2
    //     uint32_t dram_addr = (uintptr_t)A;
    //     uint32_t spm_tile_idx = 0;
    //     uint32_t rs1 = dram_addr;
    //     uint32_t rs2 = pack_dma_rs2(spm_tile_idx, A_DRAM_STRIDE);  // 128 bytes per i16 tile
    //     LOADMAT_A_8TILE(rd, rs1, rs2);
    //     asm volatile("fence");
    // }
    // {
    //     // For i16 elements: col offset = tile_col * TILE_SIZE * 2
    //     uint32_t dram_addr = (uintptr_t)B;
    //     uint32_t spm_tile_idx = 0;
    //     uint32_t rs1 = dram_addr;
    //     uint32_t rs2 = pack_dma_rs2(spm_tile_idx, B_DRAM_STRIDE);  // 128 bytes per i16 tile
    //     LOADMAT_B_8TILE(rd, rs1, rs2);
    //     asm volatile("fence");
    // }
    // {
    //     uint32_t rs1 = pack_gemm_rs1(0, 0, 0);
    //     uint32_t rs2 = pack_gemm_rs2(0, 0);
    //     GEMM8X8_I8(rd, rs1, rs2);
    //     asm volatile("fence");
    // }
    // {
    //     uint32_t rs1 = pack_gemm_rs1(1, 0, 1);
    //     uint32_t rs2 = pack_gemm_rs2(1, 1);
    //     GEMM8X8_I8(rd, rs1, rs2);
    //     asm volatile("fence");
    // }
    // {
    //     // For i16 elements: col offset = tile_col * TILE_SIZE * 2
    //     uint32_t dram_addr = (uintptr_t)D;
    //     uint32_t spm_tile_idx = 0;
    //     uint32_t rs1 = dram_addr;
    //     uint32_t rs2 = pack_dma_rs2(spm_tile_idx, C_DRAM_STRIDE);  // 128 bytes per i16 tile
    //     STOREMAT_C_8TILE(rd, rs1, rs2);
    //     asm volatile("fence");
    // }
    // for (int i = 0; i < 8; i++) {
